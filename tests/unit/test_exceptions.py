...
class TestExceptions:
    ...
    def test_invalid_channel_format_message(self) -> None:
    ...
    def test_invalid_channel_format_custom_format(self) -> None:
        """Test InvalidChannelFormat with custom expected format."""
        error = InvalidChannelFormat("wrong_format", expected_format="Type:SubType:ID")

        assert "wrong_format" in str(error)
        assert "Expected: 'Type:SubType:ID'" in str(error)
        assert error.expected_format == "Type:SubType:ID"

    def test_message_class_not_registered_without_suggestions(self) -> None:
    ...
    def test_message_class_not_registered_with_suggestions(self) -> None:
        ...
        assert "UnknownMessage" in str(error)
        assert "Available classes: PaymentMessage, OrderMessage, UserMessage" in str(
            error
        )
        assert "Did you forget to register" in str(error)
        assert error.available_classes == available

    def test_message_validation_error_basic(self) -> None:
        """Test MessageValidationError with basic information."""
        error = MessageValidationError("PaymentMessage", "amount must be positive")
    ...
    def test_message_validation_error_with_payload(self) -> None:
    ...
    def test_invalid_agent_error(self) -> None:
    ...
    def test_no_agent_found_error(self) -> None:
